...
import {
  fadeUp, riseUp, scaleIn, slideLeft, stagger,
...
/* A scroll-triggered fade wrapper — used throughout */
function Reveal({
  children,
  className = '',
  variants = fadeUp,
...
function Countdown() {
  ...
  return (
    ...
    >
      {parts.map(([n, l]) => (
        <motion.div key={l} variants={fadeUp} className="flex flex-col items-center min-w-[72px]">
          <AnimatePresence mode="popLayout">
            <motion.span
              key={`${l}-${n}`}
              className="font-[var(--font-display)] font-medium text-[56px] leading-none text-sage-700 tabular-nums"
      ...
      {FAQS.map((f, i) => (
        ...
        >
          ...
          >
            ...
            <motion.span
              className="shrink-0 w-6 h-6 relative text-gold-700"
  ...
  return (
    <div className="relative bg-sage-500 overflow-hidden py-16 px-8 text-center">
      ...
      <div className="relative max-w-[640px] mx-auto">
        {eyebrow && (
          <Reveal delay={0}>
            <p className="font-[var(--font-ui)] text-[12px] tracking-[0.34em] uppercase text-gold-300 mb-3">{eyebrow}</p>
          </Reveal>
        )}
        ...
        {intro && (
          <Reveal delay={0.14}>
            <p className="font-[var(--font-body)] text-[17px] leading-[1.65] text-[var(--on-sage-2)] mt-4 max-w-[520px] mx-auto">{intro}</p>
          </Reveal>
        )}
      </div>
    </div>
    ...
    <div>
      ...
      <section className="relative bg-sage-500 overflow-hidden py-16 px-8 pb-20">
        ...
        <div className="relative max-w-[1100px] mx-auto text-center">
          <motion.div
            ...
            animate="visible"
          >
            {/* Inner content staggers after frame appears */}
            <motion.div
              variants={stagger(0.3, 0.13)}
              initial="hidden"
              animate="visible"
            >
              <motion.p variants={fadeUp} className="font-[var(--font-ui)] text-[13px] tracking-[0.34em] uppercase text-gold-300 mb-4">
                Together with their families
              </motion.p>
              <motion.p variants={fadeUp} className="font-[var(--font-script)] text-[30px] leading-[1.4] text-gold-300 mb-2">
                the wedding of
              </motion.p>
              <motion.h1
                variants={riseUp}
              ...
              <motion.div variants={fadeUp} className="my-6">
                <GoldDivider label="Saturday the 10th July, 2027" width={70} animate={false} />
              </motion.div>
              <motion.p variants={fadeUp} className="font-[var(--font-ui)] text-[13px] tracking-[0.28em] uppercase text-[var(--on-sage-2)]">
                Hobbit Hill &middot; Clitheroe
              </motion.p>
              <motion.p variants={fadeUp} className="font-[var(--font-script)] text-[32px] leading-none text-[var(--on-sage-1)] mt-7">
      ...
      <section className="bg-cream py-24 px-8">
        <div className="max-w-[720px] mx-auto text-center">
          ...
          <Reveal variants={riseUp} delay={0.08}>
      ...
      <section className="bg-cream px-8 py-20">
        <div className="max-w-[960px] mx-auto grid gap-14" style={{ gridTemplateColumns: '1.1fr 0.9fr' }}>
          {/* Timeline — each item slides in from left */}
          <ol className="list-none m-0 p-0 relative">
            {/* Animated connecting line */}
            <motion.span
              className="absolute left-[7px] top-2 bottom-2 w-px bg-sage-300 origin-top"
              initial={{ scaleY: 0 }}
            ...
            {STORY.map((s, i) => (
              ...
              >
                {/* Dot pulses in */}
                <motion.span
                  className="absolute left-0 top-1.5 w-[15px] h-[15px] rounded-full bg-cream border-2 border-gold-500"
                  initial={{ scale: 0 }}
                  whileInView={{ scale: 1 }}
                  viewport={VIEWPORT}
      ...
      <section className="bg-cream px-8 py-20">
        <motion.div
          className="max-w-[760px] mx-auto flex flex-col gap-4"
        ...
        >
          {EVENTS.map((e, i) => (
            <motion.div
              key={e.title}
              className="flex items-stretch overflow-hidden bg-cream-bright border border-sage-200 rounded-[8px] shadow-md"
              variants={fadeUp}
              transition={{ delay: i * 0.07 } as object}
            >
      ...
      <section className="bg-cream px-8 py-20">
        <motion.div
          className="max-w-[900px] mx-auto grid grid-cols-2 gap-5"
          variants={stagger(0, 0.1)}
          initial="hidden"
          whileInView="visible"
          viewport={VIEWPORT}
      ...
      <section className="bg-cream px-8 py-20">
        ...
        >
          {[{ ar: '3/4' },{ ar: '1/1' },{ ar: '3/4' },{ ar: '1/1' },{ ar: '3/4' },{ ar: '4/5' },{ ar: '4/5' },{ ar: '3/4' },{ ar: '1/1' }].map((t, i) => (
            <motion.div
              key={i}
              className="break-inside-avoid mb-4 p-2 bg-cream-bright border border-sage-200 shadow-sm"
              variants={{
                hidden: { opacity: 0, scale: 0.96 },
                visible: { opacity: 1, scale: 1, transition: { duration: 0.6, ease: EASE_OUT_EXPO, delay: i * 0.05 } },
              }}
            >
        ...
        <Reveal delay={0.2}>
          <div className="text-center mt-8">
            <p className="font-[var(--font-body)] italic text-[16px] text-fg3 mb-4">After the wedding, we&apos;ll add a shared album here for everyone&apos;s snaps.</p>
